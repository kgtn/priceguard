...
### Добавить интерактивный туториал по настройке бота

1. Создать систему состояний туториала:
   - Определить класс TutorialStates
   - Реализовать состояния для каждого шага (intro, api_keys, ozon_key, wb_key и т.д.)
   - Добавить сохранение прогресса в базе данных

2. Реализовать сообщения и клавиатуры:
   - Создать TUTORIAL_MESSAGES с текстами для каждого шага
   - Создать TUTORIAL_KEYBOARDS с кнопками навигации
   - Добавить интерактивные элементы (кнопки, ссылки)
   - Реализовать HTML-форматирование сообщений

3. Добавить обработчики в handlers/tutorial.py:
   - Создать start_tutorial для запуска туториала
   - Реализовать tutorial_next/back.ьшыы для навигации
   - Добавить обработку специальных действий
   - Интегрировать с существующими командами

4. Улучшить пользовательский опыт:
   - Добавить кнопку "Начать туториал" в /start и /help
   - Реализовать возможность пропуска/возврата к туториалу
   - Добавить контекстные подсказки на основе прогресса
   - Интегрировать проверки API ключей

5. Тестирование и отладка:
   - Проверить все сценарии прохождения туториала
   - Протестировать сохранение прогресса
   - Проверить корректность подсказок
   - Убедиться в работоспособности навигации

Ожидаемый результат:
- Пошаговый туториал для новых пользователей
- Интерактивные инструкции по настройке
- Контекстные подсказки на основе действий
- Возможность вернуться к туториалу в любой момент

### Добавить систему напоминаний для неактивных пользователей

1. Обновить схему базы данных:
   - ✅ Добавить поле last_reminder_sent в таблицу users
   - ✅ Добавить поле setup_status (started, api_added, api_validated)
   - ✅ Вместо поля registration_date использовать created_at

2. Создать сервис напоминаний (services/reminder.py):
   - ✅ Реализовать ReminderService для управления напоминаниями
   - ✅ Добавить методы для определения статуса настройки
   - ✅ Создать шаблоны сообщений для разных случаев
   - ✅ Реализовать логику отправки напоминаний

3. Добавить периодические задачи:
   - ✅ Проверка пользователей без API ключей (через 24 часа после регистрации)
   - ✅ Проверка пользователей с невалидными ключами (через 12 часов после последней попытки)
   - ✅ Повторные напоминания через увеличивающиеся интервалы
   - ✅ Ограничение максимального количества напоминаний

4. Реализовать различные типы напоминаний:
   - ✅ Для пользователей без API ключей
   - ✅ Для пользователей с невалидными ключами

5. Добавить интерактивные кнопки в напоминания:
   - ✅ "🔄 Попробовать снова"
...
Ожидаемый результат:
- Автоматические напоминания для пользователей с незавершенной настройкой
- Помощь в решении проблем с API ключами
- Увеличение конверсии в активных пользователей
- Возможность отключить напоминания

---
Все пункты плана реализованы полностью.